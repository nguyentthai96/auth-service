...
- **ORM selection**: Spring Data JPA, Hibernate, jOOQ, MyBatis, Spring JdbcClient
- **Schema-first vs Code-first**: JPA entities, Flyway SQL migrations, Liquibase changelogs
- **Migration tools**: Flyway, Liquibase, JPA DDL auto (dev only)
- **Query builders**: jOOQ type-safe queries, JPA Criteria API, QueryDSL
- **Connection management**: HikariCP pooling, transaction handling, session management
- **Performance patterns**: Eager loading, lazy loading, `@EntityGraph`, batch fetching, N+1 prevention
- **Type safety**: JPA Metamodel, jOOQ generated classes, compile-time safety
