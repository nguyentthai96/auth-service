...
## Spring Boot 4.x Architecture Review Checklist

When reviewing Spring Boot projects, verify:
...
### Data Layer
- [ ] DTOs (Java records) used for API input/output — never JPA entities
- [ ] `@Transactional(readOnly = true)` on read-only methods
- [ ] N+1 queries prevented (`@EntityGraph`, `JOIN FETCH`, or batch loading)
...
### API Design
- [ ] `@ApiVersion` or URL versioning applied
- [ ] `@Valid` on all request body parameters
- [ ] `Page<T>` for all collection endpoints
- [ ] SpringDoc OpenAPI annotations on controllers

### Anti-Patterns to Flag
- ❌ Field injection (`@Autowired` on fields)
- ❌ `ObjectMapper` usage (use `JsonMapper` in Jackson 3.0)
- ❌ `org.springframework.lang.NonNull` (use JSpecify)
- ❌ Returning JPA entities from controllers
- ❌ Business logic in controllers
- ❌ `SELECT *` or missing pagination
