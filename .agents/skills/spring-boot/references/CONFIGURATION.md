...
# Database & JPA — see data.md for complete configuration
# spring.datasource.* and spring.jpa.* are documented in data.md
...
> See [data.md](data.md) § "Connection Pool Sizing (HikariCP)" for complete HikariCP tuning guide with sizing formula.
...
```properties
# Use Testcontainers with PostgreSQL (recommended — matches production DB)
...
> See [testing.md](testing.md) for complete test configuration, `@MockitoBean`, and Testcontainers patterns.
> See [data.md](data.md) § "Testcontainers Integration" for `@ServiceConnection` setup.
