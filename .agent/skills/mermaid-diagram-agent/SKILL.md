...
description: |
  Generate professional-grade Mermaid diagram documentation packages for AI Agent Systems.
  Supports: LangGraph, AutoGen, CrewAI, Swarm, Pydantic AI, DSPy, Semantic Kernel, LlamaIndex,
  ReAct, RAG pipelines, and custom architectures.
  Output: complete deliverable package with system inventory, styled Mermaid diagrams,
...
- Documenting AI Agent Systems (LangGraph, AutoGen, CrewAI, Pydantic AI, DSPy, etc.)
- Visualizing multi-agent orchestration, routing, and coordination logic
- Creating architecture documentation for agent-based projects
- Analyzing agent system code and generating diagram packages
- Documenting RAG pipelines, ReAct loops, and swarm/handoff patterns

## When NOT to Use

- For general enterprise diagrams (API flows, ERD, deployment) → use `mermaid-diagram-enterprise/`
- For architecture pattern documentation without Mermaid → use `software-architecture/`
- For full technical documentation packages → use `docs-architect/` with this skill as support
...
**Generated:** [current date]  **Analyzer:** Claude (mermaid-diagram-agent skill)
...
| Skill | Relationship |
|-------|-------------|
| `mermaid-diagram-enterprise/` | General enterprise diagrams (API, ERD, deployment, business) — use for non-agent systems |
| `software-architecture/` | C4 Model, ADR templates, quality attributes — provides architecture context for diagrams |
| `docs-architect/` | Technical documentation architecture — orchestrates full doc packages that include diagrams |
| `design-orchestration/` | Meta-skill routing — invokes this skill when agent-system design needs visualization |
| `architecture-patterns/` | Clean/Hexagonal/Onion patterns — provides structural context for architecture diagrams |