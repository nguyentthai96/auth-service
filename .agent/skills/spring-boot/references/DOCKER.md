...
3. PostgreSQL 17 Alpine (smaller, more secure)
...
Add these services to your `compose.yaml` for a complete development stack:
...
> Spring Boot auto-detects Redis and Mail services via `@ServiceConnection` when `spring-boot-docker-compose` is on the classpath.
...
Complete stack with PostgreSQL 17 database and Spring Boot application.

**Location**: Copy to your project root

**Features**:

1. PostgreSQL 17 Alpine (lightweight, production-ready)
...
Complete stack using GraalVM 25 native image with PostgreSQL 17.
...
- PostgreSQL 17 Alpine for smaller footprint
...
- Use Alpine or slim variants: `postgres:17-alpine`, `eclipse-temurin:25-jre-alpine`
...
- **Pin specific versions** in production (postgres:17-alpine, not latest)
...
- [ ] **Pin versions**: Use specific tags (postgres:17-alpine, not latest)
