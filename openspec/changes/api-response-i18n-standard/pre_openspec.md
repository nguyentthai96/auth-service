...
> **Classification Evidence**: `I18nConfig` → `shared/config/I18nConfig.kt`; `AuthControllerAdvice` + `MessageSource` → `shared/exception/GlobalExceptionHandler.kt`; `DatabaseMessageSource` → `shared/i18n/DatabaseMessageSource.kt`; `CqrsAuthController.messageSource` → `auth/adapter/in/web/CqrsAuthController.kt`; `SessionController.messageSource` → `auth/adapter/in/web/SessionController.kt`; `AccountLifecycleController.messageSource` → `auth/adapter/in/web/AccountLifecycleController.kt`; `MfaController.messageSource` → `auth/adapter/in/web/MfaController.kt`; `SsoController.messageSource` → `auth/adapter/in/web/SsoController.kt`; `AdminSessionController.messageSource` → `auth/adapter/in/web/AdminSessionController.kt`; `RateLimitAdminController.messageSource` → `auth/adapter/in/web/RateLimitAdminController.kt`; `TokenController.messageSource` → `auth/adapter/in/web/TokenController.kt`; `LoginRateLimitFilter.messageSource` → `auth/adapter/in/web/filter/LoginRateLimitFilter.kt`; `auth-messages*.properties` → `src/main/resources/messages/`; `ClientMetadataFilter` → `auth/adapter/in/web/filter/ClientMetadataFilter.kt`; `ContentLanguageFilter` → `auth/adapter/in/web/filter/ContentLanguageFilter.kt`
> **Archive**: `openspec/changes/archive/2026-08-25-api-response-i18n-standard/pre_openspec.md`
> **Quality Score**: 96/100
> **_Generated**: 2026-08-27
> **Mode**: DELTA (archive found, code evidence updated)
...
Hệ thống hiện tại đã implement ~99% backend infrastructure: `I18nConfig`, `DatabaseMessageSource`, `AuthControllerAdvice` (với MessageSource), `ClientMetadataFilter`, `ContentLanguageFilter`, message bundles (en + vi, 77 lines/file, 55+ message keys), `LoginRateLimitFilter` (đã i18n). [CHANGED] **18 action endpoints** across **9 controllers** đã dùng `messageSource.getMessage()`: `CqrsAuthController` (5 endpoints), `SessionController` (2), `AccountLifecycleController` (3), `MfaController` (2 — confirmTotp, regenerateRecoveryCodes), `SsoController` (2 — linkIdentity, unlinkIdentity), `AdminSessionController` (1 — forceRevokeUserSessions), `RateLimitAdminController` (1 — adminUnlock), `TokenController` (1 — revokeAllSessions), `LoginRateLimitFilter` (1 — writeRateLimitResponse). Tất cả issue từ archive trước đã RESOLVED. Phần còn lại (~0.5 developer-day): frontend header injection + cleanup hardcoded error messages, legacy `AuthController` cleanup (optional).
...
- **Evidence**: `I18nConfig.kt:49-63` (CompositeMessageSource), `DatabaseMessageSource.kt` (Caffeine cache, maxSize=500, 5-min TTL), `auth-messages.properties` (77 lines, 55+ keys) + `auth-messages_vi.properties` (77 lines, 55+ keys) — **ĐÃ IMPLEMENT**
...
### FR-005: Success response với i18n message [URD] [CHANGED]
- **Actor**: Auth Service
- **Action**: Hệ thống phải trả success response với `message` field i18n server-side cho tất cả action endpoints
- **Validation**: Logout → `{"message": "Đã đăng xuất thành công"}`; Register → `{"message": "Đăng ký thành công"}`; Session revoked → `{"message": "Đã thu hồi phiên"}`; TOTP confirmed → `{"message": "Cài đặt TOTP đã được xác nhận thành công"}`
- **Evidence**:
  - `CqrsAuthController` dùng `messageSource.getMessage()` cho register (line 93), logout (line 193), change-password (line 205), forgot-password (line 213), switch-domain (line 247) — **5 endpoints**
  - `SessionController` dùng `messageSource.getMessage()` cho revokeSession (line 44), revokeAllSessions (line 59) — **2 endpoints**
  - `AccountLifecycleController` dùng `messageSource.getMessage()` cho deactivateAccount (line 40), requestDeletion (line 58), cancelDeletion (line 76) — **3 endpoints**
  - [CHANGED] `MfaController` dùng `messageSource.getMessage()` cho confirmTotp (line 56), regenerateRecoveryCodes (line 118) — **2 endpoints**
  - [CHANGED] `SsoController` dùng `messageSource.getMessage()` cho linkIdentity (line 48), unlinkIdentity (line 57) — **2 endpoints**
  - [CHANGED] `AdminSessionController` dùng `messageSource.getMessage()` cho forceRevokeUserSessions (line 87) — **1 endpoint**
  - [CHANGED] `RateLimitAdminController` dùng `messageSource.getMessage()` cho adminUnlock (line 58) — **1 endpoint**
  - [CHANGED] `TokenController` dùng `messageSource.getMessage()` cho revokeAllSessions (line 63) — **1 endpoint**
  - [CHANGED] `LoginRateLimitFilter` dùng `messageSource.getMessage()` cho writeRateLimitResponse (line 127) — **1 endpoint**
  - **18/18 ACTION ENDPOINTS DONE**
  - Data-only endpoints (GET lists, GET status, POST introspect, JWKS, MFA verify, MFA resend, SSO callback, anonymous session CRUD) — không cần i18n message (trả data structures)
  - Legacy `AuthController` (non-CQRS, disabled by default) — có hardcoded English nhưng disabled, low priority cleanup
...
- **Evidence**: `ContentLanguageFilter.kt` — `OncePerRequestFilter` set `Content-Language` header on all `/api/**` responses; defense-in-depth: `AuthControllerAdvice.setContentLanguageHeader()` also sets for error responses (`GlobalExceptionHandler.kt:134-136`); `LoginRateLimitFilter.writeRateLimitResponse()` sets `Content-Language` header directly (line 141) — **ĐÃ IMPLEMENT (100% coverage)**
...
- **Evidence**: `I18nConfig.kt:42-43`: `resolver.supportedLocales = listOf(Locale.ENGLISH, Locale.forLanguageTag("vi"))`, `resolver.setDefaultLocale(Locale.ENGLISH)` — **ĐÃ IMPLEMENT**; `LoginRateLimitFilter.resolveLocale()` mirrors same logic with `Locale.LanguageRange.parse()` + supported = [en, vi] — `LoginRateLimitFilter.kt:115-124` — consistent with I18nConfig
...
- **Evidence**: `AcceptHeaderLocaleResolver` là stateless (Spring built-in) → inherently idempotent; `ContentLanguageFilter` reads from `LocaleContextHolder` (thread-local) — deterministic; `LoginRateLimitFilter.resolveLocale()` is pure function (no state) — deterministic — **ĐÃ IMPLEMENT**
...
- ⚠️ Assumption: Data-only endpoints (GET lists, GET status, introspect, JWKS, MFA verify, SSO callback, anonymous CRUD) không cần success i18n message — Lý do: chúng trả structured data, không có user-facing message field.
- ⚠️ Assumption: RBAC/PBAC CRUD controllers không cần i18n success message — Lý do: trả domain entities, không có `message` field. Error handling covered by AuthControllerAdvice.
- ⚠️ Assumption: `InternalApiController` (service-to-service) không cần i18n — Lý do: consumers là services, không phải humans.
...
| Đầy đủ (Completeness) | 25/25 | — (tất cả action endpoints đã covered) |
| Nhất quán (Consistency) | 25/25 | — |
| Kiểm thử được (Testability) | 21/25 | FR-009: "không còn hardcoded" khó verify exhaustively |
| **Tổng** | **96/100** | |
...
|---|----------|----------|-----|-------|----------------|
| 1 | Testability | -2 | FR-009 | "không còn hardcoded" khó verify exhaustively — cần grep-based check cho error message patterns trong frontend | grep-based check cho error message patterns trong frontend code |
| 2 | Testability | -2 | FR-007 | Frontend header injection chưa implement — chưa có test evidence | Add integration test verifying Accept-Language header presence |

---

## 8. Issues & Risks

> [RESOLVED] Issue #1 từ archive (2026-08-25) — MfaController hardcoded English in regenerateRecoveryCodes → `MfaController.regenerateRecoveryCodes()` now uses `messageSource.getMessage("auth.recovery_codes_warning", ...)` (line 118) with localized fallback. RESOLVED.
> [RESOLVED] Issue #1 từ archive (2026-08-22) — LoginRateLimitFilter bypass MessageSource → `LoginRateLimitFilter.writeRateLimitResponse()` now uses `messageSource.getMessage("auth.rate_limited", ...)` with proper locale resolution via `resolveLocale()` — `LoginRateLimitFilter.kt:127-132, 113-124`. RESOLVED.
> [RESOLVED] Issue #2 từ archive (2026-08-22) — missing E2EE/Anonymous message keys → all 44 error codes + 11+ success keys đều có trong cả en + vi bundles. RESOLVED.

Không phát hiện vấn đề mới.

## 9. Open Questions

Không có câu hỏi mở. Các OQ từ research/archive đã được resolved:
...
- ~~OQ-003: Which remaining endpoints need i18n?~~ → All 18 action endpoints now covered. Data-only endpoints confirmed skip. RESOLVED.
...
- Shared Library (base-core) — reference only, no change needed
...
- `auth-service/shared/config` — I18nConfig [EXISTING]
...
- `auth-service/auth/adapter/in/web` — CqrsAuthController [EXISTING — i18n done], SessionController [EXISTING — i18n done], AccountLifecycleController [EXISTING — i18n done], MfaController [EXISTING — i18n done], SsoController [EXISTING — i18n done], AdminSessionController [EXISTING — i18n done], RateLimitAdminController [EXISTING — i18n done], TokenController [EXISTING — i18n done]
- `auth-service/auth/adapter/in/web/filter` — ClientMetadataFilter [EXISTING], ContentLanguageFilter [EXISTING], LoginRateLimitFilter [EXISTING — i18n done]
- `auth-service/resources/messages` — message bundles [EXISTING — 77 lines en+vi, 55+ keys, all needed keys present]
...
| 5a | Auth Service | `LoginRateLimitFilter.resolveLocale()` parses `Accept-Language` directly (pre-DispatcherServlet) | OncePerRequestFilter (pre-auth) |
...
| 7c | Auth Service | (Rate Limited) `LoginRateLimitFilter` → `messageSource.getMessage("auth.rate_limited", ...)` → build `ProblemDetail` | Filter → MessageSource |
...
| FR-005 | Research: UC-003 | TBD | `CqrsAuthController.kt` [DONE], `SessionController.kt` [DONE], `AccountLifecycleController.kt` [DONE], `MfaController.kt` [DONE], `SsoController.kt` [DONE], `AdminSessionController.kt` [DONE], `RateLimitAdminController.kt` [DONE], `TokenController.kt` [DONE], `LoginRateLimitFilter.kt` [DONE] | ✅ Implemented [CHANGED from 🟡 Partial] |
| FR-006 | Research: UC-001,003 | TBD | `ContentLanguageFilter.kt` [EXISTING], `AuthControllerAdvice` [EXISTING], `LoginRateLimitFilter` [EXISTING] | ✅ Implemented |
...
| FR-012 | Enriched | TBD | `I18nConfig.kt` [EXISTING], `LoginRateLimitFilter.resolveLocale()` [EXISTING] | ✅ Implemented |
| FR-013 | Enriched | TBD | `AcceptHeaderLocaleResolver` [EXISTING], `LoginRateLimitFilter.resolveLocale()` [EXISTING] | ✅ Implemented |

## 13. Agent Notes (Tổng hợp bổ sung)

### Observations
Feature này thuộc dạng **cross-cutting concern** — ảnh hưởng toàn bộ API layer nhưng thay đổi chủ yếu ở infrastructure level (config, advice, filter). Rủi ro cực thấp vì:

1. **99% backend infrastructure đã implement** [CHANGED from 97%]: `I18nConfig`, `DatabaseMessageSource`, `AuthControllerAdvice` (MessageSource integration), `ClientMetadataFilter` (MDC logging), `ContentLanguageFilter` (100% Content-Language coverage), message bundles (en + vi, 77 lines, 55+ keys covering all 44+ error codes + 11+ success messages)
2. [CHANGED] **18 action endpoints** across **9 controllers** đã dùng `messageSource.getMessage()` — pattern proven across all action controller types: main auth (CqrsAuthController), session management (SessionController, AdminSessionController), account lifecycle (AccountLifecycleController), MFA (MfaController), SSO (SsoController), rate limiting (RateLimitAdminController, LoginRateLimitFilter), token management (TokenController)
3. [RESOLVED] All issues from previous archives — MfaController now uses MessageSource (line 118), LoginRateLimitFilter fully i18n-enabled, all message keys present in both bundles
4. `AuthErrorCode` enum now has **62+ error codes** (AUTH_001-AUTH_062) — all with `msgCode` convention matching message bundle keys
5. Message bundles have **77 lines each** (en + vi) — covering all error codes + success messages + anonymous + E2EE + event sourcing + inter-service error codes

**DELTA Changes from Archive (2026-08-25 → 2026-08-27):**

| Section | Change Type | Detail |
|---------|------------|--------|
| FR-005 Evidence | [CHANGED] | 10/20+ → 18/18 action endpoints. Added MfaController (2), SsoController (2), AdminSessionController (1), RateLimitAdminController (1), TokenController (1) |
| FR-005 Status | [CHANGED] | 🟡 Partial → ✅ Implemented |
| Issue #1 (MfaController) | [RESOLVED] | MfaController.regenerateRecoveryCodes() now uses messageSource with key `auth.recovery_codes_warning` |
| Quality Score | [CHANGED] | 94 → 96 (all action endpoints covered, no remaining completeness gap) |
| Issues count | [CHANGED] | 1 → 0 (all resolved) |
| Classification Evidence | [CHANGED] | Added MfaController, SsoController, AdminSessionController, RateLimitAdminController, TokenController references |
| Traceability Matrix | [CHANGED] | Updated FR-005 status to ✅ Implemented |

**Remaining work (~0.5 developer-day):**
...
- Optional: legacy `AuthController` i18n cleanup (disabled by default, lowest priority)
...
- `auth-core-features` (archived: `openspec/changes/archive/2026-08-20-auth-core-features/`) — added `SessionController`, `AccountLifecycleController` with MessageSource integration.
- `erp-iam-system` (archived: `openspec/changes/archive/2026-08-24-erp-iam-system/`) — added RBAC/PBAC modules (controllers without i18n — confirmed skip).
...
- **LoginRateLimitFilter**: Uses `MessageSource.getMessage()` for i18n ProblemDetail. Has own `resolveLocale()` because it runs pre-DispatcherServlet (LocaleContextHolder not populated). Mirrors I18nConfig supported locales [en, vi].
...
### Suggested Approach
1. **Frontend ONLY** (backend complete):
...
2. **Optional cleanup**:
   - Legacy `AuthController` (non-CQRS, disabled) — add i18n for completeness or remove entirely
3. **base-core (no change needed)**: Auth-service handles all i18n via `AuthControllerAdvice`
...
Change Impact:
  ...
  FR-005 → [REUSE] CqrsAuthController.kt, SessionController.kt, AccountLifecycleController.kt, MfaController.kt, SsoController.kt, AdminSessionController.kt, RateLimitAdminController.kt, TokenController.kt, LoginRateLimitFilter.kt → all 18 action endpoints done
  FR-006 → [REUSE] ContentLanguageFilter.kt, LoginRateLimitFilter.kt → no changes needed
