...
> [CHANGED] Updated 2026-08-25 — reflects current codebase state where ~97% infrastructure is implemented AND LoginRateLimitFilter i18n is resolved. Remaining: extend success i18n to 7 action endpoints + frontend changes.
...
**Current implementation status**: ~97% complete. Infrastructure (`I18nConfig`, `DatabaseMessageSource`, `AuthControllerAdvice`, `ContentLanguageFilter`, `ClientMetadataFilter`, `LoginRateLimitFilter` i18n, message bundles 55 keys en+vi, controller i18n for 10 action endpoints + 1 filter) is all EXISTING. Remaining: extend success i18n to 7 action endpoints across 5 controllers + frontend header injection + form cleanup.
...
### FR-005: Success response với i18n message [URD] — PARTIALLY IMPLEMENTED [CHANGED]
...
- **Remaining (7 action endpoints)**:
  - `MfaController.confirmTotp()` — add `"message"` to response map
  - `MfaController.regenerateRecoveryCodes()` — replace hardcoded English `"warning"` with i18n
  - `TokenController.revokeAllSessions()` — wrap DTO in map + add message
  - `SsoController.linkIdentity()` — add `"message"` to response map
  - `SsoController.unlinkIdentity()` — add `"message"` to response map
  - `AdminSessionController.forceRevokeUserSessions()` — replace hardcoded English `"message"` with i18n
  - `RateLimitAdminController.adminUnlock()` — wrap DTO in map + add message
- **Skip (data-only)**: All GET endpoints, login/refresh (return AuthResponse tokens), introspect (RFC 7662), JWKS, anonymous endpoints (data/204), internal endpoints (machine consumers), RBAC/PBAC controllers (entity CRUD), MfaController.updateSettings (data IS confirmation), MfaController.verifyMfa (returns auth response), MfaController.setupTotp (returns data), MfaController.resendOtp (returns MFA data)
- **Error**: N/A
...
### New Message Keys (FR-005 extension)

| Key | EN | VI |
|-----|----|----|
| `auth.totp_confirmed` | TOTP setup confirmed successfully | Xác nhận cài đặt TOTP thành công |
| `auth.recovery_codes_warning` | Save these codes — they will not be shown again. | Lưu lại các mã này — chúng sẽ không được hiển thị lại. |
| `auth.all_user_sessions_revoked` | All sessions revoked | Đã thu hồi tất cả phiên |
| `auth.sso_identity_linked` | SSO identity linked successfully | Liên kết danh tính SSO thành công |
| `auth.sso_identity_unlinked` | SSO identity unlinked successfully | Hủy liên kết danh tính SSO thành công |
| `auth.sessions_revoked_for_user` | All sessions revoked for user {0} | Đã thu hồi tất cả phiên cho người dùng {0} |
| `auth.rate_limit_unlocked` | Rate limit locks cleared successfully | Đã xóa khóa giới hạn tốc độ thành công |
...
| (success) sessions_revoked_for_user | [42] | All sessions revoked for user 42 | Đã thu hồi tất cả phiên cho người dùng 42 |
