...
> **Classification Evidence**: `I18nConfig` → `shared/config/I18nConfig.kt`; `AuthControllerAdvice` + `MessageSource` → `shared/exception/GlobalExceptionHandler.kt`; `DatabaseMessageSource` → `shared/i18n/DatabaseMessageSource.kt`; `CqrsAuthController.messageSource` → `auth/adapter/in/web/CqrsAuthController.kt`; `SessionController.messageSource` → `auth/adapter/in/web/SessionController.kt`; `AccountLifecycleController.messageSource` → `auth/adapter/in/web/AccountLifecycleController.kt`; `LoginRateLimitFilter.messageSource` → `auth/adapter/in/web/filter/LoginRateLimitFilter.kt`; `auth-messages*.properties` → `src/main/resources/messages/`; `ClientMetadataFilter` → `auth/adapter/in/web/filter/ClientMetadataFilter.kt`; `ContentLanguageFilter` → `auth/adapter/in/web/filter/ContentLanguageFilter.kt`
> **Archive**: `openspec/changes/archive/2026-08-22-api-response-i18n-standard/pre_openspec.md`
...
Hệ thống hiện tại đã implement ~97% infrastructure: `I18nConfig`, `DatabaseMessageSource`, `AuthControllerAdvice` (với MessageSource), `ClientMetadataFilter`, `ContentLanguageFilter`, message bundles (en + vi, 70 lines/file), `LoginRateLimitFilter` (đã i18n). [CHANGED] `CqrsAuthController` đã dùng `messageSource.getMessage()` cho 5 endpoints + `SessionController` (2 endpoints) + `AccountLifecycleController` (3 endpoints) = **10 endpoints done**. `LoginRateLimitFilter` đã refactored để dùng `MessageSource` (Issue #1 từ archive đã RESOLVED). Phần còn lại (~1 developer-day): success message i18n cho remaining controllers (MfaController, TokenController, AnonymousAuthController, SsoController, AdminSessionController, RateLimitAdminController, InternalApiController, RBAC/PBAC controllers), frontend header injection + cleanup hardcoded error messages.
...
- **Validation**: Logout → `{"message": "Đã đăng xuất thành công"}`; Register → `{"message": "Đăng ký thành công"}`; Session revoked → `{"message": "Đã thu hồi phiên"}`; Account deactivated → `{"message": "Tài khoản đã bị vô hiệu hóa..."}`
- **Evidence**: 
  - `CqrsAuthController` dùng `messageSource.getMessage()` cho register (line 89), logout (line 189), change-password (line 201), forgot-password (line 209), switch-domain (line 243) — **5 endpoints**
  - [NEW] `SessionController` dùng `messageSource.getMessage()` cho revokeSession (line 44), revokeAllSessions (line 59) — **2 endpoints**
  - [NEW] `AccountLifecycleController` dùng `messageSource.getMessage()` cho deactivateAccount (line 40), requestDeletion (line 58), cancelDeletion (line 76) — **3 endpoints**
  - **10/20+ ENDPOINTS DONE**; cần mở rộng cho MfaController (7 endpoints), TokenController (3 endpoints), AnonymousAuthController (5 endpoints), SsoController, AdminSessionController, RateLimitAdminController, InternalApiController, RBAC/PBAC controllers
...
- **Evidence**: `ContentLanguageFilter.kt` — `OncePerRequestFilter` set `Content-Language` header on all `/api/**` responses (line 37-39); defense-in-depth: `AuthControllerAdvice.setContentLanguageHeader()` also sets for error responses (`GlobalExceptionHandler.kt:134-136`); [CHANGED] `LoginRateLimitFilter.writeRateLimitResponse()` now sets `Content-Language` header directly (line 141) — **ĐÃ IMPLEMENT (100% coverage)**
...
- ⚠️ Assumption: Data-only endpoints (GET lists, GET status, introspect) không cần success i18n message — Lý do: chúng trả structured data, không có user-facing message.
...
|----------|-------|-----------|
| Rõ ràng (Clarity) | 25/25 | — (FR-008 regex pattern đã bổ sung từ archive) |
| Đầy đủ (Completeness) | 23/25 | FR-005: 10/20+ endpoints done, remaining endpoints cần list cụ thể |
| Nhất quán (Consistency) | 25/25 | — |
| Kiểm thử được (Testability) | 21/25 | FR-005: cần example values cho mỗi remaining endpoint; FR-009: "không còn hardcoded" khó verify exhaustively |
| **Tổng** | **94/100** | |
...
|---|----------|----------|-----|-------|----------------|
| 1 | Completeness | -2 | FR-005 | 10/20+ endpoints done — remaining chưa list rõ ràng từng endpoint | List remaining: MfaController (confirmTotp, updateSettings, regenerateRecoveryCodes), TokenController (revokeAllSessions), SsoController (callback), Admin controllers |
| 2 | Testability | -2 | FR-005 | Thiếu expected i18n message example cho remaining action endpoints | Thêm expected message keys + values cho mỗi action endpoint |
| 3 | Testability | -2 | FR-009 | "không còn hardcoded" khó verify exhaustively | grep-based check cho error message patterns trong frontend |
...
|---|------|--------|-------|-----|---------|
| 1 | Risk | 🟡 | `MfaController.regenerateRecoveryCodes()` trả hardcoded English: `"Save these codes — they will not be shown again."` (line 100) — bypass MessageSource i18n | FR-005 | Add message key `auth.recovery_codes_warning` to bundles; inject MessageSource vào MfaController |

> [RESOLVED] Issue #1 từ archive (LoginRateLimitFilter bypass MessageSource) — `LoginRateLimitFilter.writeRateLimitResponse()` now uses `messageSource.getMessage("auth.rate_limited", ...)` with proper locale resolution via `resolveLocale()` — `LoginRateLimitFilter.kt:127-132, 113-124`
> [RESOLVED] Issue #2 từ previous version (missing E2EE/Anonymous message keys) — all 44 error codes + 11 success keys đều có trong cả en + vi bundles (unchanged from archive)
...
- **auth-service**: Primary — `AuthControllerAdvice` (MessageSource-integrated), `CqrsAuthController` (5 endpoints i18n-enabled), `SessionController` [NEW] (2 endpoints i18n-enabled), `AccountLifecycleController` [NEW] (3 endpoints i18n-enabled), `LoginRateLimitFilter` [CHANGED] (now i18n-enabled), `I18nConfig`, `DatabaseMessageSource`, `ClientMetadataFilter`, `ContentLanguageFilter`, message bundles (70 lines en+vi, 55+ keys). Cần extend success i18n cho remaining controllers.
...
- Keyword: `messageSource.getMessage` → Module: `auth/adapter/in/web` → File: `CqrsAuthController.kt` (lines 89, 189, 201, 209, 243)
- [NEW] Keyword: `messageSource.getMessage` → Module: `auth/adapter/in/web` → File: `SessionController.kt` (lines 44, 59)
- [NEW] Keyword: `messageSource.getMessage` → Module: `auth/adapter/in/web` → File: `AccountLifecycleController.kt` (lines 40, 58, 76)
- [CHANGED] Keyword: `messageSource.getMessage` → Module: `auth/adapter/in/web/filter` → File: `LoginRateLimitFilter.kt` (line 127)
...
- `auth-service/auth/adapter/in/web` — CqrsAuthController [EXISTING — i18n done], SessionController [EXISTING — i18n done], AccountLifecycleController [EXISTING — i18n done]
- `auth-service/auth/adapter/in/web` — MfaController [MODIFY — add i18n for confirmation responses], TokenController [MODIFY — add i18n for revoke response], AnonymousAuthController [MODIFY — add i18n for create/renew responses], SsoController [MODIFY — add i18n for callback response]
- `auth-service/auth/adapter/in/web` — AdminSessionController [MODIFY — add i18n for admin action responses], RateLimitAdminController [MODIFY — add i18n for admin action responses], InternalApiController [MODIFY — add i18n for service-to-service responses]
...
- `auth-service/resources/messages` — message bundles [EXISTING — may need new keys for remaining endpoints]
...
| 5a | Auth Service | [CHANGED] `LoginRateLimitFilter.resolveLocale()` parses `Accept-Language` directly (pre-DispatcherServlet) | OncePerRequestFilter (pre-auth) |
...
| 7c | Auth Service | [CHANGED] (Rate Limited) `LoginRateLimitFilter` → `messageSource.getMessage("auth.rate_limited", ...)` → build `ProblemDetail` | Filter → MessageSource |
...
| FR-005 | Research: UC-003 | TBD | `CqrsAuthController.kt` [EXISTING], `SessionController.kt` [NEW-i18n], `AccountLifecycleController.kt` [NEW-i18n], MfaController [MODIFY], TokenController [MODIFY], others [MODIFY] | 🟡 Partial (10/20+) [CHANGED from 5/12] |
| FR-006 | Research: UC-001,003 | TBD | `ContentLanguageFilter.kt` [EXISTING], `AuthControllerAdvice` [EXISTING], `LoginRateLimitFilter` [CHANGED — now sets header] | ✅ Implemented |
...
| FR-012 | Enriched | TBD | `I18nConfig.kt` [EXISTING], `LoginRateLimitFilter.resolveLocale()` [CHANGED — consistent] | ✅ Implemented |
| FR-013 | Enriched | TBD | `AcceptHeaderLocaleResolver` [EXISTING — stateless], `LoginRateLimitFilter.resolveLocale()` [CHANGED — pure function] | ✅ Implemented |
...
1. **97%+ infrastructure đã implement** [CHANGED from 95%]: `I18nConfig`, `DatabaseMessageSource`, `AuthControllerAdvice` (MessageSource integration), `ClientMetadataFilter` (MDC logging), `ContentLanguageFilter` (100% Content-Language coverage), message bundles (en + vi, 70 lines, 55+ keys covering all 44 error codes + 11 success messages)
2. [CHANGED] `CqrsAuthController` (5 endpoints) + `SessionController` (2 endpoints) + `AccountLifecycleController` (3 endpoints) = **10 endpoints** đã dùng `messageSource.getMessage()` — pattern đã proven across 3 controllers
3. [RESOLVED] `LoginRateLimitFilter` đã refactored — now uses `MessageSource.getMessage("auth.rate_limited", ...)` with proper locale resolution via `resolveLocale()` method that parses `Accept-Language` directly (pre-DispatcherServlet, since `LocaleContextHolder` isn't populated yet). Supported locales mirror `I18nConfig`: [en, vi].
4. `AuthErrorCode.msgCode` convention match message bundle keys exactly (e.g., `auth.rate_limited` → `auth-messages.properties`)
5. `AcceptHeaderLocaleResolver` + `ContentLanguageFilter` + `ClientMetadataFilter` → full request/response i18n pipeline đã hoạt động

**DELTA Changes from Archive (2026-08-22 → 2026-08-25):**

| Section | Change Type | Detail |
|---------|------------|--------|
| FR-005 Evidence | [CHANGED] | 5/12 → 10/20+ endpoints. Added SessionController (2), AccountLifecycleController (3) |
| FR-006 Evidence | [CHANGED] | LoginRateLimitFilter now sets Content-Language header directly |
| FR-012 Evidence | [CHANGED] | LoginRateLimitFilter.resolveLocale() mirrors I18nConfig locales |
| FR-013 Evidence | [CHANGED] | LoginRateLimitFilter.resolveLocale() added as pure function |
| Issue #1 | [RESOLVED] | LoginRateLimitFilter now uses MessageSource for i18n |
| Issue #1 (new) | [NEW] | MfaController hardcoded English in regenerateRecoveryCodes |
| Quality Score | [CHANGED] | 92 → 94 (fewer remaining gaps) |
| Classification Evidence | [CHANGED] | Added SessionController, AccountLifecycleController, LoginRateLimitFilter references |
| Transaction Flow | [CHANGED] | Added steps 5a, 7c for LoginRateLimitFilter locale resolution |
| Traceability Matrix | [CHANGED] | Updated FR-005 (10/20+), FR-006 (LoginRateLimitFilter), FR-012, FR-013 |

**Remaining work (~1 developer-day):**
- Extend success i18n cho remaining controllers:
  - `MfaController`: confirmTotp, updateSettings, regenerateRecoveryCodes (3 confirmation endpoints)
  - `TokenController`: revokeAllSessions (1 action endpoint)
  - `SsoController`: ssoCallback (1 — action response, if confirmation needed)
  - `AdminSessionController`: forceRevokeUserSessions (1 action endpoint)
  - `RateLimitAdminController`: resetLock (1 action endpoint)
  - Data-only endpoints (GET list, GET status, POST introspect, JWKS) — **do not need i18n message**, chỉ trả data structures
- Add message keys to bundles for new action endpoints
...
- [NEW] `auth-core-features` (archived: `openspec/changes/archive/2026-08-20-auth-core-features/`) — added `SessionController`, `AccountLifecycleController` with MessageSource integration.
- [NEW] `erp-iam-system` (archived: `openspec/changes/archive/2026-08-24-erp-iam-system/`) — added RBAC/PBAC modules (controllers without i18n).
...
- **LoginRateLimitFilter**: [CHANGED] Now uses `MessageSource.getMessage()` for i18n ProblemDetail. Has own `resolveLocale()` because it runs pre-DispatcherServlet (LocaleContextHolder not populated). Mirrors I18nConfig supported locales [en, vi].
...
1. **auth-service backend FIRST**: 
   - Extend success i18n cho remaining controllers with confirmation/action responses (MfaController, TokenController, SsoController, AdminSessionController, RateLimitAdminController)
   - Skip i18n for data-only endpoints (GET lists, GET status, POST introspect, JWKS, etc.)
   - Add success message keys to bundles for new action endpoints
2. **frontend SECOND**:
...
Change Impact:
  ...
  FR-003 → [REUSE] I18nConfig, DatabaseMessageSource, auth-messages*.properties → may need new message keys
  FR-004 → [REUSE] AuthControllerAdvice (GlobalExceptionHandler.kt) → no changes needed
  FR-005 → [MODIFY] MfaController.kt, TokenController.kt, SsoController.kt, AdminSessionController.kt, RateLimitAdminController.kt, InternalApiController.kt → extend success i18n for action endpoints
  FR-005 → [REUSE] CqrsAuthController.kt, SessionController.kt, AccountLifecycleController.kt → already done
