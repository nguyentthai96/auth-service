...
<!-- generated-at: 2026-08-22T12:00:00+07:00 -->
<!-- updated: 2026-08-22 — reflects ~97% implementation, LoginRateLimitFilter gap + frontend pending -->
...
>
> [CHANGED] v2.0: Updated to reflect current implementation state. ~97% backend complete. Only LoginRateLimitFilter i18n gap remains.
...
|--------|--------|--------|-------------|---------------|
| `Content-Language` | ✅ | BCP 47 | Actual locale used to render response messages | `ContentLanguageFilter` (all paths) + `AuthControllerAdvice` (errors) + `LoginRateLimitFilter` (rate limit — [PENDING]) |
...
| `en` | `AUTH_020` | `"Too many login attempts (IP). Please wait 60 seconds."` | LoginRateLimitFilter | 🔲 PENDING |
| `vi` | `AUTH_020` | `"Quá nhiều lần đăng nhập (IP). Vui lòng đợi 60 giây."` | LoginRateLimitFilter | 🔲 PENDING |
...
| `en` | `AUTH_041` (args=[1024,2048]) | `"Data size (1024 bytes) exceeds the maximum allowed (2048 bytes)"` | AuthControllerAdvice | ✅ |
| `vi` | `AUTH_041` (args=[1024,2048]) | `"Kích thước dữ liệu (1024 bytes) vượt quá giới hạn cho phép (2048 bytes)"` | AuthControllerAdvice | ✅ |
...
### Success Message Keys — ✅ ALL 11 KEYS IN BUNDLES

| Key | Endpoint | EN | VI |
|-----|----------|----|----|
| `auth.login_success` | (reserved — data-only endpoint) | Login successful | Đăng nhập thành công |
...
| `auth.token_refreshed` | (reserved — data-only endpoint) | Token refreshed successfully | Làm mới token thành công |
...
- [x] All 11 success message keys documented
...
- [x] LoginRateLimitFilter i18n gap identified with fix specification
- [x] Contract rules defined (9 rules)
